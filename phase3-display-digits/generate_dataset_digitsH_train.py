...
output_folder = './digitsH_train/' #'./new_digits_yolo/' #end it with /
...
first_transforms_set = Sequence([RandomScale((-0.7,-0.6)),RandomRotate(1)])
...
for epoch in range(num_epochs): #a complete sweep over all digits
    for n in range(N): #go over all files
        ...
        if os.path.basename(file_name) == '00111.png':
            print('jddj')

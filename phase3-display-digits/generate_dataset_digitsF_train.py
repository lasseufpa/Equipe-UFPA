...
num_epochs = 500 #number of runs over all digits. There are 10 digits, so if you choose 2, you end up with 2*10 output images
...
random_center_shift = 12 #number of pixels the center of the image will be shifted, from - to +
...
output_folder = './digitsF_train/' #'./new_digits_yolo/' #end it with /
os.makedirs(output_folder, exist_ok=True)

total_number_of_noise_patches = 3
max_number_of_pixels_in_noise_patches = 10 #num_pixels_final_x # 10 for 10 x 10 patches
number_of_pixels_away_from_display = 30
...
for epoch in range(num_epochs): #a complete sweep over all digits
    for n in range(N): #go over all files
        ...
        #add noise
        if should_add_noise:
            if random.random() > 0.3: #only add noise in some images
                if random.random() > 0.5:
                    img_single_digit = poisson_noise(img_single_digit, poisson_factor)
                else:
                    img_single_digit = gaussian_noise(img_single_digit, noise_sigma)

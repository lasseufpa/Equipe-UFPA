...
def fill_weights_torch_model(weights, state_dict):
    for name in state_dict:
        if name == "classifier.weight":
            ...
            key = (
                name.replace("features", "MobilenetV1")
                .replace(".", "/")
                .replace("BatchNorm/weight", "BatchNorm/gamma")
            )
            ...
            key = (
                name.replace("features", "MobilenetV1")
                .replace(".", "/")
                .replace("BatchNorm/bias", "BatchNorm/beta")
            )
            ...
            key = (
                name.replace("features", "MobilenetV1")
                .replace(".", "/")
                .replace("running_mean", "moving_mean")
            )
            ...
            key = (
                name.replace("features", "MobilenetV1")
                .replace(".", "/")
                .replace("running_var", "moving_variance")
            )
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Usage: python translate_tf_modelnetv1.py <tf_model.pb> <pytorch_weights.pth>"
        )
    tf_model = sys.argv[1]

...
class ModelPrunner:
    ...
    def remove_conv_filter(self, path, filter_index):
        ...
        logging.info(
            f'Prune Conv: {"/".join(path)}, Filter: {filter_index}, Layer: {conv}'
        )
        ...
        if next_conv_path:
            ...
            new_next_conv = self._make_new_conv(
                next_conv, filter_index, channel_type="in"
            )
        ...
        if linear_path:
            ...
            new_linear = self._make_new_linear(
                linear, filter_index, conv, channel_type="in"
            )
    ...
    def register_conv_hooks(self):
        ...
        def forward_hook(m, input, output):
            ...
            if isinstance(m, nn.Conv2d):
                ...
                self.last_conv_path = (
                    None  # after a linear layer the conv layer doesn't matter
                )
        ...
        for path, m in self.book.modules(
            module_type=(nn.Conv2d, nn.BatchNorm2d, nn.Linear)
        ):
    ...
    def remove_linear_feature(self, path, feature_index):
        ...
        logging.info(
            f'Prune Linear: {"/".join(path)}, Filter: {feature_index}, Layer: {linear}'
        )
        ...
        if next_linear_path:
            ...
            new_next_linear = self._make_new_linear(
                next_linear, feature_index, channel_type="in"
            )

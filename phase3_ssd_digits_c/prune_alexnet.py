...
parser.add_argument(
    "--num_epochs", default=25, type=int, help="number of batches to train"
)
parser.add_argument(
...
parser.add_argument(
    "--recovery_batch_size", default=32, type=int, help="Batch size for training"
)
...
parser.add_argument(
    "--window", default=10, type=int, help="Window size for tracking training accuracy."
)
...
def train_epoch(net, data_iter, num_epochs=1, optimizer=None):
    ...
    train_accuracy /= num
    logging.info(
        "Train Epoch Loss:{:.4f}, Accuracy:{:.4f}".format(train_loss, train_accuracy)
    )
    ...
    for i in range(num_epochs):
        ...
        logging.info(
            "Epoch: {}, Training Loss:{:.4f}, Training Accuracy:{:.4f}".format(
                i, running_loss / num, running_corrects / num
            )
        )
        ...
        if save_model:
            torch.save(
                net.state_dict(),
                "models/ant-alexnet-epoch-{}-{:.4f}.pth".format(i, val_accuracy),
            )
...
def make_prunner_loader(dataset):
    loader = torch.utils.data.DataLoader(
        dataset, batch_size=args.recovery_batch_size, shuffle=True, num_workers=1
    )
    while True:
        for inputs, labels in loader:
            yield inputs, labels
...
if __name__ == "__main__":
    ...
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=len(val_dataset), shuffle=False, num_workers=1
    )
    ...
    if args.train:
        ...
        prunner = ModelPrunner(
            net,
            lambda model: train_epoch(model, prunner_data_iter),
            ignored_paths=[("classifier", "6")],
        )  # do not prune the last layer.
        ...
        if args.prune_conv:
            prune_num = prunner.book.num_of_conv2d_filters() - 5 * (
                prunner.book.num_of_conv2d_modules()
            )
        else:
            prune_num = prunner.book.num_of_linear_filters() - 5 * (
                prunner.book.num_of_linear_modules()
            )
        ...
        while i < prune_num:
            ...
            if iteration % 10 == 0:
                ...
                logging.info(
                    f"Prune: {i}/{prune_num}, After Pruning Evaluation Accuracy:{val_accuracy:.4f}."
                )
            val_loss, val_accuracy = train_epoch(
                prunner.model, train_data_iter, args.num_recovery_batches, optimizer
            )
            ...
            if iteration % 10 == 0:
                ...
                val_loss, val_accuracy = eval(prunner.model, val_loader)
                logging.info(
                    f"Prune: {i}/{prune_num}, After Recovery Evaluation Accuracy:{val_accuracy:.4f}."
                )
                logging.info(
                    f"Prune: {i}/{prune_num}, Iteration: {iteration}, Save model."
                )
                with open(f"models/alexnet-pruned-{i}.txt", "w") as f:
                    print(prunner.model, file=f)
                torch.save(
                    prunner.model.state_dict(),
                    f"models/prunned-alexnet-{i}-{prune_num}-{val_accuracy:.4f}.pth",
                )
            iteration += 1
    else:
